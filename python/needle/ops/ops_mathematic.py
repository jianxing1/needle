...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        a = node.inputs[0]
        output_shape = [1] * (len(self.shape) - len(a.shape)) + list(a.shape)
        axes = tuple([i for i in range(len(self.shape)) 
              if (i >= len(output_shape) or output_shape[i] != self.shape[i])])
...
class Summation(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        # if self.axes is None:
        #   output_shape = [1 for i in range(len(input_shape))]
        # else:
        #   output_shape = [1 if (i in self.axes) else input_shape[i] for i in range(len(input_shape))]
        # grad = reshape(out_grad, output_shape)
        # grad = broadcast_to(grad, input_shape)
        output_shape = list(input_shape)
        axes = range(len(output_shape)) if self.axes is None else self.axes
        for axis in axes:
          output_shape[axis] = 1
        grad = reshape(out_grad, output_shape)
        grad = broadcast_to(grad, input_shape)
