...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(size_t i = shape.size; i > 0; i--){
      size_t idx = gid_temp % shape.data[i-1];
      gid_temp /= shape.data[i-1];
      gid_a += idx * strides.data[i-1];
    }
...
__global__ void MatmulKernel(const scalar_t* A, const scalar_t* B, scalar_t* C, int row_a, int col_a, int row_b, int col_b) {

  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;

  __shared__ float As[thread_num_y][thread_num_x+1];
  __shared__ float Bs[thread_num_y][thread_num_x+1];

  float sum = 0;
  int num_iter = ceil((float)col_a / thread_num_x);
  for(int i = 0; i < num_iter; i++){
    if(i*blockDim.x + threadIdx.x < col_a && row < row_a){
      As[threadIdx.y][threadIdx.x] = A[row*col_a + i*blockDim.x + threadIdx.x];
    }else{
      As[threadIdx.y][threadIdx.x] = 0;
    }
    if(i*blockDim.y + threadIdx.y < row_b && col < col_b){
      Bs[threadIdx.y][threadIdx.x] = B[(i*blockDim.y + threadIdx.y)*col_b + col];
    }else{
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 grid(ceil((float)P / thread_num_x), ceil((float)M / thread_num_y));
  MatmulKernel<<<grid, block>>>(a.ptr, b.ptr, out->ptr, M, N, N, P);
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  if(gid < size){
    scalar_t max_out = INT_MIN;
    for(size_t i = 0; i < reduce_size; i++){
      max_out = max(max_out, a[gid * reduce_size + i]);
    }
    out[gid] = max_out;
  }
  /// END SOLUTION
